...
namespace UltrasharpTools.Tools.Services {
    public class SemanticSimilarityService(
        ...
        private readonly IComplexityAnalysisService _complexityAnalysisService = complexityAnalysisService ?? throw new ArgumentNullException(nameof(complexityAnalysisService));
        ...
        private async Task<MethodSemanticFeatures?> ExtractFeaturesAsync(
            ...
            await _complexityAnalysisService.AnalyzeMethodAsync(methodSymbol, methodMetrics, recommendations, cancellationToken, compilation);
        ...
        private List<MethodSimilarityResult> CompareFeatures(
            ...
            _logger.LogInformation("Parallel similarity analysis complete. Found {GroupCount} groups.", results.Count);
        ...
        private List<ClassSimilarityResult> CompareClassFeatures(
            ...
            _logger.LogInformation("Parallel class similarity analysis complete. Found {GroupCount} groups.", results.Count);
            return results.OrderByDescending(r => r.AverageSimilarityScore).ToList();
        }
