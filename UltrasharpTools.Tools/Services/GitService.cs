...
public class GitService(ILogger<GitService> logger, GitOptions? gitOptions = null) : IGitService
{
    ...
    public async Task<List<string>> CleanupOldBranchesAsync(string solutionPath, int? retentionCount = null, int? retentionDays = null, CancellationToken cancellationToken = default) {
        return await Task.Run(() => {
            ...
            try {
                ...
                var sharpToolsBranches = repository.Branches
                    .Where(b => b.FriendlyName.StartsWith(SharpToolsBranchPrefix, StringComparison.OrdinalIgnoreCase))
                    .OrderByDescending(b => b.Tip.Author.When)
                    .ToList();

                _logger.LogDebug("Found {Count} SharpTools branches for cleanup evaluation", sharpToolsBranches.Count);

                // Calculate retention cutoff date
                DateTime? cutoffDate = retentionDays.HasValue
                    ? DateTime.UtcNow.AddDays(-retentionDays.Value)
                    : null;

                for (int i = 0; i < sharpToolsBranches.Count; i++) {
                    var branch = sharpToolsBranches[i];

                    // Never delete the current branch
                    if (branch.FriendlyName == currentBranch) {
                        _logger.LogDebug("Skipping current branch: {BranchName}", branch.FriendlyName);
                        continue;
                    }
                    ...
                    if (retentionCount.HasValue && i >= retentionCount.Value) {
                        shouldDelete = true;
                        reason = $"beyond retention count {retentionCount.Value}";
                    }

                    // Check retention days
                    if (cutoffDate.HasValue && branch.Tip.Author.When.UtcDateTime < cutoffDate.Value) {
                        shouldDelete = true;
                    ...
                    if (shouldDelete) {
                        try {
                            ...
                            _logger.LogInformation("Deleted SharpTools branch: {BranchName} ({Reason})",
                                branch.FriendlyName, reason);
                        } catch (Exception ex) {
                            _logger.LogWarning(ex, "Failed to delete branch {BranchName}", branch.FriendlyName);
                        }
                    }
                }

                if (deletedBranches.Count > 0) {
                    _logger.LogInformation("Cleanup complete: deleted {Count} SharpTools branches", deletedBranches.Count);
                } else {
                    _logger.LogDebug("No branches deleted during cleanup");
                }

                return deletedBranches;
            } catch (Exception ex) {
                _logger.LogError(ex, "Error during branch cleanup for solution at {SolutionPath}", solutionPath);
                return deletedBranches;
            }
        }, cancellationToken);
    }
    ...
    public async Task<string> GetCurrentBranchAsync(string solutionPath, CancellationToken cancellationToken = default) {
        return await Task.Run(() => {
            try {
                var repositoryPath = GetRepositoryPath(solutionPath);
                if (repositoryPath == null) {
                    _logger.LogDebug("No Git repository found for solution at {SolutionPath}", solutionPath);
                    return string.Empty;
                }
                ...
                _logger.LogWarning(ex, "Error getting current branch for solution at {SolutionPath}", solutionPath);
                return string.Empty;
            }
        }, cancellationToken);
    }
    ...
    public async Task<string> GetCurrentCommitShaAsync(string solutionPath, CancellationToken cancellationToken = default) {
        return await Task.Run(() => {
            try {
                ...
                using var repository = new Repository(repositoryPath);
                return repository.Head.Tip?.Sha ?? string.Empty;
            } catch (Exception ex) {
                _logger.LogWarning(ex, "Error getting current commit SHA for solution at {SolutionPath}", solutionPath);
                return string.Empty;
            }
        }, cancellationToken);
    }
    ...
    public async Task<string?> GetMergeBaseCommitAsync(string solutionPath, string branch1, string branch2, CancellationToken cancellationToken = default) {
        return await Task.Run(() => {
            try {
                ...
                var commit1 = repository.Branches[branch1]?.Tip ?? repository.Lookup<Commit>(branch1);
                var commit2 = repository.Branches[branch2]?.Tip ?? repository.Lookup<Commit>(branch2);

                if (commit1 == null || commit2 == null) {
                    _logger.LogWarning("Could not find commits for branches: {Branch1}, {Branch2}", branch1, branch2);
                    return null;
                }

                // Find merge base (common ancestor)
                var mergeBase = repository.ObjectDatabase.FindMergeBase(commit1, commit2);

                if (mergeBase == null) {
                    _logger.LogWarning("No merge base found between {Branch1} and {Branch2}", branch1, branch2);
                    return null;
                }

                _logger.LogDebug("Merge base found: {MergeBaseSha} for branches {Branch1} and {Branch2}",
                    mergeBase.Sha[..8], branch1, branch2);

                return mergeBase.Sha;
            } catch (Exception ex) {
                _logger.LogError(ex, "Error finding merge base between {Branch1} and {Branch2} for solution at {SolutionPath}",
                    branch1, branch2, solutionPath);
                return null;
            }
        }, cancellationToken);
    }
    ...
    public async Task<List<(string FilePath, string ChangeType)>> GetChangedFilesAsync(
        string solutionPath, string fromCommitSha, string toCommitSha, CancellationToken cancellationToken = default) {
        return await Task.Run(() => {
            ...
            try {
                ...
                var fromCommit = repository.Lookup<Commit>(fromCommitSha);
                var toCommit = repository.Lookup<Commit>(toCommitSha);

                if (fromCommit == null || toCommit == null) {
                    _logger.LogWarning("Could not find commits: From={FromSha}, To={ToSha}",
                        fromCommitSha?[..8] ?? "null", toCommitSha?[..8] ?? "null");
                    return changedFiles;
                }

                // Compare trees to get changes
                var comparison = repository.Diff.Compare<TreeChanges>(fromCommit.Tree, toCommit.Tree);

                foreach (var change in comparison) {
                    // Map LibGit2Sharp ChangeKind to our ChangeType strings
                    ...
                    changedFiles.Add((change.Path, changeType));
                }

                _logger.LogDebug("Found {Count} changed files between {FromSha} and {ToSha}",
                    changedFiles.Count, fromCommitSha[..8], toCommitSha[..8]);

                return changedFiles;
            } catch (Exception ex) {
                _logger.LogError(ex, "Error getting changed files between {FromSha} and {ToSha} for solution at {SolutionPath}",
                    fromCommitSha, toCommitSha, solutionPath);
                return changedFiles;
            }
        }, cancellationToken);
    }
        ...
        return await Task.Run(() => {
            ...
            try {
                ...
                foreach (var branch in repository.Branches) {
                    // Add branch name (remove origin/ prefix if remote)
                    ...
                    if (!branches.Contains(branchName, StringComparer.OrdinalIgnoreCase)) {
                        branches.Add(branchName);
                    }
                }

                _logger.LogDebug("Found {Count} unique branches in repository", branches.Count);
                return branches;
            } catch (Exception ex) {
