...
public async Task ConnectAsync(CancellationToken cancellationToken = default) {
    ...
    try {
        ...
        if (IsVectorDBProcessRunning()) {
            // Процесс существует но медленно отвечает (idle mode) - retry с большим таймаутом
            try {
                _logger.LogInformation("[VectorDB] VectorDB process exists but slow to respond (idle mode?), retrying with longer timeout...");
                _pipeClient = new NamedPipeClientStream(".", PipeName, PipeDirection.InOut, PipeOptions.Asynchronous);
                await _pipeClient.ConnectAsync(30000, cancellationToken); // 30 секунд для idle mode wake-up
                InitializeStreams();
                _logger.LogInformation("[VectorDB] Connected to existing VectorDB process (after wake-up)");
                return;
            } catch (TimeoutException) {
                // Даже 30 секунд не хватило - процесс завис, убиваем и запускаем новый
                _pipeClient?.Dispose();
                _pipeClient = null;
                _logger.LogWarning("[VectorDB] Process not responding after 30s, killing and restarting...");
                KillVectorDBProcess();
            }
        }
