...
public static partial class FileOperationTools
{
    ...
    /// </summary>
    [McpServerTool(Name = "split_file", Idempotent = false, ReadOnly = false, Destructive = false, OpenWorld = false)]
    [Description("Split large C# file into separate files, one per top-level type (class/interface/enum/struct). " +
    ...
    public static async Task<object> SplitFile(
        ...
        [Description("Absolute path to .cs file to split")]
        string filePath,

        [Description("Target directory for split files (absolute)")]
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            ErrorHandlingHelpers.ValidateStringParameter(targetDirectory, nameof(targetDirectory), logger);
            ...
            logger.LogInformation("Splitting file: {FilePath} -> {TargetDir} (preview={Preview})",
                filePath, targetDirectory, preview);
            ...
            var newFiles = allMembers.Select(member =>
            {
                string typeName;
                string typeKind;
                ...
                var lineCount = member.GetLocation().GetLineSpan().Span.End.Line -
                                member.GetLocation().GetLineSpan().Span.Start.Line + 1;
            ...
            foreach (var member in allMembers)
            {
                ...
                var parentNamespace = member.Ancestors().OfType<BaseNamespaceDeclarationSyntax>().FirstOrDefault();
                if (parentNamespace != null)
                {
                    var newNamespace = SyntaxFactory.FileScopedNamespaceDeclaration(parentNamespace.Name)
                        .AddMembers(member);
            ...
            {
                logger.LogInformation("Optimizing imports in {Count} files...", createdFiles.Count);
                ...
                foreach (var file in createdFiles)
                {
                    try
                    {
                        ...
                        logger.LogDebug("Optimized imports for {File}: +{Added} -{Removed}",
                            Path.GetFileName(file), updateResult.UsingsAdded.Count, updateResult.UsingsRemoved.Count);
                    }
                    catch (Exception ex)
                    {
            ...
            return ToolHelpers.ToJson(new
            {
                ...
                message = $"Successfully split into {createdFiles.Count} files" +
                         (optimizeImports ? $", optimized imports in {importResults.Count(r => ((dynamic)r).success)} files" : "")
            });

        }, logger, nameof(SplitFile), cancellationToken);
    }

    /// <summary>
    /// Synthesize (combine) multiple C# files into a single file.
    /// </summary>
    [McpServerTool(Name = "synthesize_files", Idempotent = false, ReadOnly = false, Destructive = false, OpenWorld = false)]
    [Description("Combine multiple C# files into a single file. Merges usings and types, preserving namespaces. " +
    ...
    public static async Task<object> SynthesizeFiles(
        ...
        [Description("Array of absolute paths to .cs files to combine")]
        string[] filePaths,

        [Description("Target file path (absolute)")]
        string targetFilePath,

        [Description("Preview mode (default: true, shows combined code)")]
        bool preview = true,
        ...
        CancellationToken cancellationToken = default)
    {
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () =>
        {
            ...
            ErrorHandlingHelpers.ValidateStringParameter(targetFilePath, nameof(targetFilePath), logger);
            ...
            logger.LogInformation("Synthesizing {Count} files into {Target} (preview={Preview})",
                filePaths.Length, targetFilePath, preview);
            ...
            {
                var nsDecl = SyntaxFactory.FileScopedNamespaceDeclaration(SyntaxFactory.ParseName(nsName))
            ...
            if (preview)
            {
                return ToolHelpers.ToJson(new
                {
                    ...
                    preview = combinedCode.Length > 10000
                        ? combinedCode.Substring(0, 10000) + "\n\n... (truncated, total " + lineCount + " lines)"
                        : combinedCode,
            ...
            if (optimizeImports)
            {
                ...
                try
                {
                    ...
                    logger.LogInformation("Optimized imports: +{Added} -{Removed}",
                        updateResult.UsingsAdded.Count, updateResult.UsingsRemoved.Count);
                }
                catch (Exception ex)
                {
            ...
            return ToolHelpers.ToJson(new
            {
                ...
                message = $"Successfully synthesized {filePaths.Length} files into {targetFilePath}" +
                         (optimizeImports && importOptimization != null ? ", imports optimized" : "")
            });

        }, logger, nameof(SynthesizeFiles), cancellationToken);
    }
}
