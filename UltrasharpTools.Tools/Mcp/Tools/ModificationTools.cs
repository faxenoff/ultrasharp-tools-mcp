...
public static class ModificationTools {
    ...
    public static async Task<string> AddMember(
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            ...
            try {
                ...
                var lintingResult = await modificationService.ApplyChangesAsync(newSolution, cancellationToken, finalCommitMessage);
    ...
    public static async Task<string> ReplaceAllReferences(
        ...
        [Description("Only replace symbols in files with this pattern. Supports globbing (`*`). Empty = all files.")] string filenameFilter = "",
        [Description("Only replace specific syntax node types. Options: 'variable', 'method', 'class', 'property', 'field', 'invocation', 'parameter', 'type'. Empty array = all types. Example: ['invocation', 'variable']")] string[]? syntaxNodeKinds = null,
        [Description("If true, exclude replacements in comments and strings (default: true)")] bool excludeCommentsAndStrings = true,
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            ...
            // Ensure solution is loaded
            await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(solutionManager, logger, nameof(ReplaceAllReferences), cancellationToken);
            logger.LogInformation("Executing '{ReplaceAllReferences}' for {SymbolName} with text '{ReplacementCode}', filter: {Filter}, preview: {Preview}",
                nameof(ReplaceAllReferences), fullyQualifiedSymbolName, replacementCode, filenameFilter ?? "none", preview);
            ...
            if (!string.IsNullOrEmpty(filenameFilter)) {
                ...
                fileRoot = Path.GetPathRoot(solutionManager.CurrentSolution?.FilePath) ?? Path.GetPathRoot(Environment.CurrentDirectory)!;
            }

            // Setup syntax node kind filter
            HashSet<string>? nodeKindSet = null;
            if (syntaxNodeKinds != null && syntaxNodeKinds.Length > 0) {
                nodeKindSet = new HashSet<string>(syntaxNodeKinds, StringComparer.OrdinalIgnoreCase);

                // Validate node kinds
                var validKinds = new[] { "variable", "method", "class", "property", "field", "invocation", "parameter", "type", "identifier" };
                foreach (var kind in syntaxNodeKinds) {
                    if (!validKinds.Contains(kind.ToLowerInvariant())) {
                        logger.LogWarning("Unknown syntax node kind: {Kind}. Valid kinds: {ValidKinds}",
                            kind, string.Join(", ", validKinds));
                    }
                }
            }
                ...
                predicateFilter = node => {
                    try {
                        ...
                        if (excludeCommentsAndStrings) {
                            ...
                            var current = node;
                            while (current != null) {
                                if (current.IsKind(SyntaxKind.StringLiteralExpression) ||
                                    ...
                                    current.IsKind(SyntaxKind.SingleLineDocumentationCommentTrivia) ||
                                    current.IsKind(SyntaxKind.MultiLineDocumentationCommentTrivia)) {
                                    return false;
                                }
                        ...
                        if (nodeKindSet != null) {
                            ...
                            if (nodeKindSet.Contains("variable") &&
                                (node is VariableDeclaratorSyntax || node.Parent is VariableDeclaratorSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("method") &&
                                     (node is MethodDeclarationSyntax || node.Parent is MethodDeclarationSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("class") &&
                                     (node is ClassDeclarationSyntax || node.Parent is ClassDeclarationSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("property") &&
                                     (node is PropertyDeclarationSyntax || node.Parent is PropertyDeclarationSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("field") &&
                                     (node is FieldDeclarationSyntax || node.Parent is FieldDeclarationSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("invocation") &&
                                     (node is InvocationExpressionSyntax || node.Parent is InvocationExpressionSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("parameter") &&
                                     (node is ParameterSyntax || node.Parent is ParameterSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("type") &&
                                     (node is TypeDeclarationSyntax || node.Parent is TypeDeclarationSyntax)) {
                                matchesKind = true;
                            }
                            else if (nodeKindSet.Contains("identifier") && node is IdentifierNameSyntax) {
                                matchesKind = true;
                            }
    ...
    public static async Task<string> ReplaceAllReferencesByPattern(
        ...
        ICodeModificationService modificationService,
        ILogger<ModificationToolsLogCategory> logger,
        [Description("Pattern to match symbol names. Supports wildcards (* and ?) or regex. Examples: 'test*', 'Old*Class', '^test\\w+'.")] string symbolNamePattern,
        [Description("Replacement pattern. Wildcards are substituted with captured parts. Examples: 'nonmod*', 'New*Class'.")] string replacementPattern,
        [Description("Pattern type: 'Auto' (default, auto-detect), 'Wildcard' (* and ?), 'Regex', or 'Exact'.")] string patternType = "Auto",
        [Description("Filter by symbol kinds. Options: 'method', 'class', 'property', 'field', 'variable', 'parameter', 'type'. Empty = all types.")] string[]? symbolKinds = null,
        [Description("Filter by namespace pattern (supports wildcards). Example: 'MyApp.Services.*'. Empty = all namespaces.")] string namespaceFilter = "",
        [Description("Filter by file pattern (supports globbing). Example: '*.cs', '*Controller.cs'. Empty = all files.")] string filenameFilter = "",
        [Description("If true, only preview changes without applying them (default: true)")] bool preview = true,
        [Description("Commit message (only used when preview=false)")] string commitMessage = "",
        CancellationToken cancellationToken = default) {
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            // Validate parameters
            ErrorHandlingHelpers.ValidateStringParameter(symbolNamePattern, nameof(symbolNamePattern), logger);
            ErrorHandlingHelpers.ValidateStringParameter(replacementPattern, nameof(replacementPattern), logger);
            ...
            // Ensure solution is loaded
            await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(solutionManager, logger, nameof(ReplaceAllReferencesByPattern), cancellationToken);

            logger.LogInformation("Executing {ToolName}: pattern '{Pattern}' → '{Replacement}', type: {Type}, preview: {Preview}",
                nameof(ReplaceAllReferencesByPattern), symbolNamePattern, replacementPattern, patternType, preview);
            ...
            PatternType parsedPatternType = patternType.ToLowerInvariant() switch
            {
                ...
                _ => throw new McpException($"Invalid patternType: {patternType}. Valid values: Auto, Wildcard, Regex, Exact")
            };
            ...
            {
                namespaceMatcher = new SymbolPatternMatcher(namespaceFilter, PatternType.Wildcard);
            }
            ...
            {
                symbolKindSet = new HashSet<string>(symbolKinds, StringComparer.OrdinalIgnoreCase);
            }
            ...
            var matchingSymbols = new List<(ISymbol Symbol, string CurrentName, string NewName)>();
            ...
            foreach (var project in solution.Projects)
            {
                ...
                foreach (var symbol in allSymbols)
                {
                    ...
                    if (!string.IsNullOrEmpty(filenameFilter))
                    {
                        ...
                        var fileRoot = Path.GetPathRoot(solution.FilePath) ?? Path.GetPathRoot(Environment.CurrentDirectory)!;
            ...
            {
                logger.LogWarning("No symbols found matching pattern '{Pattern}'", symbolNamePattern);
            ...
            {
                var previewOutput = new StringBuilder();
                previewOutput.AppendLine($"## Preview: Batch rename symbols matching '{symbolNamePattern}' → '{replacementPattern}'");
                previewOutput.AppendLine();
                ...
                if (symbolKinds != null && symbolKinds.Length > 0)
                    previewOutput.AppendLine($"• Symbol kinds: `{string.Join(", ", symbolKinds)}`");
                if (!string.IsNullOrEmpty(filenameFilter))
                    previewOutput.AppendLine($"• File filter: `{filenameFilter}`");
                previewOutput.AppendLine();
                ...
                for (int i = 0; i < displayCount; i++)
                {
                    ...
                    previewOutput.AppendLine($"  • [{symbolKind}] {ns}.{currentName} → {newName}");
                }

                if (matchingSymbols.Count > 50)
                {
                    previewOutput.AppendLine($"  ... and {matchingSymbols.Count - 50} more symbol(s)");
                }
            ...
            // Apply batch rename
            logger.LogInformation("Applying batch rename for {Count} symbols...", matchingSymbols.Count);
            ...
            if (successCount > 0)
            {
                var finalCommitMessage = $"Batch rename: '{symbolNamePattern}' → '{replacementPattern}' ({successCount} symbols): {commitMessage}";
        ...
        }, logger, nameof(ReplaceAllReferencesByPattern), cancellationToken);
    }
    ...
    private static bool MatchesSymbolKind(ISymbol symbol, HashSet<string> symbolKindSet)
    {
        if (symbolKindSet.Contains("method") && symbol.Kind == SymbolKind.Method)
            return true;
        if (symbolKindSet.Contains("class") && symbol is INamedTypeSymbol { TypeKind: TypeKind.Class })
            return true;
    ...
    public static async Task<string> Undo(
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(solutionManager, logger, nameof(Undo), cancellationToken);
    ...
    public static async Task<string> FindAndReplace(
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            ...
            // Ensure solution is loaded
            await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(solutionManager, logger, nameof(FindAndReplace), cancellationToken);
    ...
    public static async Task<string> MoveMember(
        ...
        return await ErrorHandlingHelpers.ExecuteWithErrorHandlingAsync(async () => {
            ...
            await ToolHelpers.EnsureSolutionLoadedOrAutoLoadAsync(solutionManager, logger, nameof(MoveMember), cancellationToken);
