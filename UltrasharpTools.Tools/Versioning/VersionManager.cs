...
var solutionDir = Path.GetDirectoryName(_solutionManager.CurrentSolution?.FilePath ?? Directory.GetCurrentDirectory()) ?? Directory.GetCurrentDirectory();
...
{
var solutionPath = _solutionManager.CurrentSolution?.FilePath ?? throw new InvalidOperationException("No solution loaded");
...
private async Task RollbackGitSnapshotAsync(string snapshotId, CancellationToken cancellationToken)
{
// Git stash rollback planned for future version
throw new NotSupportedException("Git stash rollback not yet implemented. Please use file backup snapshots.");
}

private async Task<List<SnapshotInfo>> ListGitSnapshotsAsync(int limit, CancellationToken cancellationToken)
{
...
_logger.LogInformation("Created file snapshot {SnapshotId} with {FileCount} files", snapshotId, backedUpFiles.Count);
return snapshotId;
}

private async Task RollbackBackupSnapshotAsync(string snapshotId, CancellationToken cancellationToken)
{
var snapshotDir = Path.Combine(_backupDir, snapshotId);
...
?? throw new InvalidOperationException($"Failed to parse snapshot metadata: {snapshotId}");
...
private async Task<List<SnapshotInfo>> ListBackupSnapshotsAsync(int limit, CancellationToken cancellationToken)
{
var snapshots = new List<SnapshotInfo>();
...
var snapshotDirs = Directory.GetDirectories(_backupDir)
...
var sizeBytes = Directory.GetFiles(snapshotDir, "*", SearchOption.AllDirectories)
...
private async Task DeleteBackupSnapshotAsync(string snapshotId, CancellationToken cancellationToken)
{
var snapshotDir = Path.Combine(_backupDir, snapshotId);
if (Directory.Exists(snapshotDir))
{
...
private async Task CopyFileAsync(string sourcePath, string targetPath, CancellationToken cancellationToken)
{
using var sourceStream = new FileStream(sourcePath, FileMode.Open, FileAccess.Read, FileShare.Read, 4096, useAsync: true);
using var targetStream = new FileStream(targetPath, FileMode.Create, FileAccess.Write, FileShare.None, 4096, useAsync: true);
...
{
_logger.LogWarning(ex, "Failed to load git stash metadata for {SnapshotId}", snapshotId);
return null;
}
}
}
