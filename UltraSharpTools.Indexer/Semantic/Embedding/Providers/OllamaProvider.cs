...
public sealed class OllamaProvider : IEmbeddingProvider
{
    ...
    )
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync(
                "/api/embeddings",
                request,
                EmbeddingJsonContext.Default.OllamaRequest,
                cancellationToken
            );
            response.EnsureSuccessStatusCode();

            var result = await response.Content.ReadFromJsonAsync(
                EmbeddingJsonContext.Default.OllamaResponse,
                cancellationToken
            );
    ...
    private async Task<bool> CheckModelExistsAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var result = await response.Content.ReadFromJsonAsync(
                EmbeddingJsonContext.Default.OllamaTagsResponse,
                cancellationToken
            );
    ...
    private async Task PullModelAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            var response = await _httpClient.PostAsJsonAsync(
                "/api/pull",
                request,
                EmbeddingJsonContext.Default.OllamaPullRequest,
                cts.Token
            );
            response.EnsureSuccessStatusCode();
