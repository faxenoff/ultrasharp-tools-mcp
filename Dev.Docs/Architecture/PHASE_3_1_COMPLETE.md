...
- `UltraSharpTools.Indexer/UltraSharpTools.Indexer.csproj` (строка 40 удалена)
...
- `Dev.Scripts/publish-indexer.ps1` (строки 33-42)
...
2. IndexerClient.SendRequestAsync()
   ↓
3. Проверяет подключение к Indexer
   ↓
4a. Подключен → отправляет JSON-RPC запрос
4b. Не подключен → запускает Indexer.exe и подключается
   ↓
5. Indexer обрабатывает запрос и возвращает результат
```
...
**UltraSharpTools.Indexer/**
- ✅ `UltraSharpTools.Indexer.csproj` - конфигурация проекта
- ✅ `Program.cs` - entry point
...
- ✅ `IndexerClient.cs` - клиент для Indexer IPC
...
- ✅ `Dev.Scripts/publish-indexer.ps1` - отключен AOT временно
...
### 2. Миграция Semantic компонентов в Indexer
...
{
    // Вернуть статус Indexer (количество embedding, версия, etc.)
}
```
...
- Делегировать всю semantic логику в Indexer
...
2. **Indexer методы не реализованы** - IndexerService содержит только TODO
   - **Status**: Будет реализовано в Phase 3.2

3. **Semantic компоненты еще в Tools** - не перенесены в Indexer
